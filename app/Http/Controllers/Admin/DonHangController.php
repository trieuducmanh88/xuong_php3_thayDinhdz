...
class DonHangController extends Controller
{
    ...
    public function update(Request $request, string $id)
    {
        ...
        if($currentTrangThai == DonHang::HUY_DON_HANG){
            return redirect()->route('admins.donhangs.index')->with('error','Đơn hàng đã bị huỷ không thể thay đổi trạng thái');
    ...
    {
        // khi người dùng huỷ đơn hàng thì mới được xoá , và xoá mềm
        $donHang = DonHang::query()->findOrFail($id);
        ...
        return redirect()->back()->with('error','Đơn hàng huỳ thì mởi có thể xoá');
